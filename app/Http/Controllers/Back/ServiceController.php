...
class ServiceController extends Controller
{
    ...
    function category_multiple_deactivate(Request $request){
    ...
    function category_multiple_activate(Request $request){
        if ($request->strIds == null) {
            session()->flash('error', 'You do not select Id!');
            return response()->json(['error' => 1]);
        } else {
    ...
    function category_multiple_delete(Request $request){
        if ($request->strIds == null) {
            session()->flash('error', 'You do not select Id!');
            return response()->json(['error' => 1]);
        } else {
    ...
    function store_service(Request $request){
    ...
    function update_service($id,Request $request){
        // validate request
        $validatedData = $request->validate([
            'name' => 'required|string|max:255',
            'min' => 'required|integer',
    ...
    function service_multiple_deactivate(Request $request){
        if ($request->strIds == null) {
            session()->flash('error', 'You do not select Id!');
            return response()->json(['error' => 1]);
        } else {
    ...
    function service_multiple_activate(Request $request){
        if ($request->strIds == null) {
            session()->flash('error', 'You do not select Id!');
            return response()->json(['error' => 1]);
        } else {
    ...
    function service_multiple_delete(Request $request){
        if ($request->strIds == null) {
            session()->flash('error', 'You do not select Id!');
            return response()->json(['error' => 1]);
        } else {
    ...
    // Edit provider
    function edit_provider($id, Request $request){
    ...
    function provider_price($id, Request $request){
        // return $request;
    ...
    function sync_exists_services($data_services = [], $params = []){
        // return $params;
        if (!empty($data_services)) {
            ...
            foreach ($data_services as $key => $item) {
                ...
                if (isset($sync_request_options['original_price']) && $sync_request_options['original_price']) {
                    $data_item['api_price'] = (double)$item['rate'];
                }
    ...
    function sync_all_services($data_services = [], $params = []){
    ...
    function import_provider_services($id, Request $request){
        $provider = ApiProvider::findOrFail($id);
    ...
    function store_provider_service(Request $request){
        // return $request;
    ...
    function provider_bulkimport(Request $request){
        // return $request->all();
        if ($request->strIds == null) {
            session()->flash('error', 'You do not select Id!');
            return response()->json(['error' => 1]);
    ...
    function get_apiServices(Request $request){
        // return $request;
        $option_plan = "";
