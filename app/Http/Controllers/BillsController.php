...
class BillsController extends Controller
{
    ...
    function data_plans(Request $request){
        ...
        if($dataplan->count() > 0 ){
            foreach($dataplan as $fetch_data){
                ...
                '<option value="default" data-price="100" id="0" > Select Data plan </option>';
            }
    ...
    function cable_plans(Request $request){
        ...
        if($cableplan->count() > 0 ){
            foreach($cableplan as $fetch_data){
                ...
                '<option value="default" data-price="100" disabled >-- Select cable plan --</option>';
            }
    ...
    function cable_discount(Request $request){
    ...
    function buy_electricity(Request $request){
        // return $request->all();
        if(sys_setting('is_power') != 1){
            return redirect()->back()->withError('Electricity Payment service is currently disabled. Please try again');
        }
    ...
    function buy_bet(Request $request){
        // return $request->all();
        if(sys_setting('is_betting') != 1){
